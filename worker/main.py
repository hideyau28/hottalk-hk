...
async def debug_assign_dry_run() -> dict[str, Any]:
    ...
    try:
        ...
        for t in all_topics:
            ...
            if isinstance(pj, str):
                try:
                    ...
                    pj = {}
            platforms_set = set(pj.keys()) if isinstance(pj, dict) else set()
            if platforms_set != {"google_trends"}:
                non_trends.append(t)
        ...
        logger.error("assign_dry_run_failed", error=str(e), tb=traceback.format_exc())
        return {
            "status": "error",
            "error_type": type(e).__name__,
            "error_message": str(e),
